...
namespace Sequlite.CameraLib
{
    ...
    public class PVCamCamera
    {
        ...
        public delegate void StartExpoHandle();
        public event StartExpoHandle StartExposure;
        ...
        Boolean m_isDefectivePixelCorrection;
        Boolean m_isDynamicDarkFrameCorrection;
        Boolean m_isEnhancedDynamicRange;
        ...
        UInt16 m_FirmwareMajorVersion;
        UInt16 m_FirmwareMinorVersion;
        ...
        public bool StartSeqAcq()
        {
            ...
            m_SingleAcqThread.Name = "PVCAM Single Acq"; // added name
            m_SingleAcqThread.Start();

            return true;
        }

        //start synchronous sequential acquisition
        public bool StartSyncSeqAcq()
        {
            m_abortAcquisition = false;
        ...
        private void SingleAcquisition()
        {
            ...
            //wait for image acquisition to be completed with polling, every 1ms check
            ...
            {
                Thread.Sleep(1);
                var counts = new List<int> { 50, 200, 500, 800, 1000, 1500, 2000, 3000, 4000, 5000 };
                count++;
                if (counts.Contains(count))
                {
                    ReportMsg?.Invoke(this, new ReportMessage(string.Format("Wait for image acquisition to be completed with polling, {0}", count), MsgTypes.MSG_STATUS));
                }
                if (count > 10000)
                {
                    ReportMsg?.Invoke(this, new ReportMessage("Failed to wait for image acquisition to be completed with polling", MsgTypes.MSG_ERROR));
                    m_acqRunning = false;
                    return;
                }
            }
        ...
        public bool StartContinuousAcquisition()
        {
            ...
            m_FrameDispThread.Name = "PVCam Get new Frame"; // added thread name
        ...
        {
            if (m_liveOn) //if the camera has been running in continuous/circular buffer acquisition mode
            {
                //stop the acquisition
            ...
            else //if the camera has been running in sequential/single/predetermined frame number mode
            {
                //abort the exposure
            ...
            //give it time for the thread to end normally
            System.Threading.Thread.Sleep(100);

            //force the thread to abort
            if (m_FrameDispThread != null)
            {
                if (m_FrameDispThread.IsAlive)
                {
                    try
                    {
                        m_FrameDispThread.Abort();
                    }
        ...
        {
            //make sure PVCAM is initialized before opening the camera
            ...
            {
                if (activeCamera.ReportMsg != null)
                {
            ...
            else
            {
                ...
                if (activeCamera.CamNotif != null)
                {
                    activeCamera.CamNotif(activeCamera, new ReportEvent(CameraNotifications.CAMERA_OPENED));
                }
            }

            //call pl_exp_init_seq() only once after opening the camera, do not call repeatedly during the acquisition
            if (!PVCAM.pl_exp_init_seq())
            {
                if (activeCamera.ReportMsg != null)
                {
                    activeCamera.ReportMsg(activeCamera, new ReportMessage("init_seq() failed", MsgTypes.MSG_ERROR));
                }
                PVCAM.pl_pvcam_uninit();
                return false;
            }
            else
            {
                if (activeCamera.ReportMsg != null)
                {
                    activeCamera.ReportMsg(activeCamera, new ReportMessage("pl_exp_seq_init() OK", MsgTypes.MSG_STATUS));
                }
            }
            return true;
        }*/
        ...
        public static Boolean OpenCamera(StringBuilder cameraToOpen, PVCamCamera activeCamera)
        {
            ...
            activeCamera.SetClearMode("Pre-Exposure");
            //set camera clear cycles number
            activeCamera.SetClearCycles(2);
            //set camera clocking mode
            activeCamera.SetClockingMode("Normal");
        ...
        private void ReadCentroidParameters()
        {
        ...
        private void ReadPostProcessingFeatures()
        {
            ...
            m_isDefectivePixelCorrection = false;
            m_isDynamicDarkFrameCorrection = false;
            m_isEnhancedDynamicRange = false;
            ...
            for (i = 0; i < featCount; i++)
            {
                ...
                if (featureID == (uint)PvTypes.PP_FEATURE_IDS.PP_FEATURE_DEFECTIVE_PIXEL_CORRECTION)
                {
                    m_isDefectivePixelCorrection = true;
                }
                else if (featureID == (uint)PvTypes.PP_FEATURE_IDS.PP_FEATURE_DYNAMIC_DARK_FRAME_CORRECTION)
                {
                    m_isDynamicDarkFrameCorrection = true;
                }
                else if (featureID == (uint)PvTypes.PP_FEATURE_IDS.PP_FEATURE_ENHANCED_DYNAMIC_RANGE)
                {
                    m_isEnhancedDynamicRange = true;
                }
        ...
        /// <summary>
        /// Configure post processing feature by feature/function id
        /// 
        /// Post processing feature is searched with Feature ID/Function ID rather than index
        /// Which might change with firmware/camera to camera
        /// </summary>
        public void ConfigPostProcessingID(PvTypes.PP_FEATURE_IDS targetfeatID, PvTypes.PP_PARAMETER_IDS targetfuncID, bool bIsEnable)
        {
            bool bIsAvail = false;
            uint featCount = 0;     //number of features available on camera 
            uint funcCount = 0;     //number of function each feature has
            int featIndex = 0;     //feature index
            uint featID = 0;        //feature id 
            int funcIndex = 0;      //function index
            uint funcID = 0;        //function ID
            string featName = string.Empty;    //feature name
            string funcName = string.Empty;    //Function name

            bool bfeatFound = false, bfuncFound = false;   //if target feature has been found

            uint curValue = 0;      //current value of function 
            int newValue = 0;      //new value that function has to be set

            //const uint ENABLE = 1;
            //const uint DISABLE = 0;
            int ENABLE = (bIsEnable) ? 1 : 0;
            ...
            //Check if Post Processing features are available on the camera

            //Get Post Processing Feature Count
            IntPtr unmngIsAvaiable = Marshal.AllocHGlobal(sizeof(UInt16));
            if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_INDEX, (Int16)PvTypes.AttributeIDs.ATTR_AVAIL, unmngIsAvaiable))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Error while checking availablity of PARAM_PP_INDEX", MsgTypes.MSG_ERROR));
                }
                m_isPostProcessingAvail = bIsAvail = false;
            }
            else
            {
                bIsAvail = Convert.ToBoolean(Marshal.ReadInt16(unmngIsAvaiable));
            }
            ...
            if (bIsAvail == false)
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Post Processing features not available on this camera", MsgTypes.MSG_ERROR));
                }
                return; //it is not a failure
            }

            //Get Post Processing Feature Count
            IntPtr unmngFeatCount = Marshal.AllocHGlobal(sizeof(UInt32));
            if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_INDEX, (Int16)PvTypes.AttributeIDs.ATTR_COUNT, unmngFeatCount))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Error while getting feature count", MsgTypes.MSG_ERROR));
                }
                return;
            }

            featCount = (UInt32)Marshal.ReadInt32(unmngFeatCount);

            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Number of Post Processing Features on camera {0}", featCount), MsgTypes.MSG_STATUS));
            }

            for (featIndex = 0; featIndex < featCount; featIndex++)
            {
                //Set current feature 
                IntPtr unmngFeatIndex = Marshal.AllocHGlobal(sizeof(UInt32));
                Marshal.WriteInt32(unmngFeatIndex, featIndex);
                if (!PVCAM.pl_set_param(m_hCam, PvTypes.PARAM_PP_INDEX, unmngFeatIndex))
                {
                    if (ReportMsg != null)
                    {
                        ReportMsg(this, new ReportMessage("Error while setting feature index", MsgTypes.MSG_ERROR));
                    }
                    return;
                }

                //get feature name & index of that feature 
                IntPtr unmngFeatureName = Marshal.AllocHGlobal(PvTypes.PARAM_NAME_LEN);
                ...
                if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_FEAT_NAME, (Int16)PvTypes.AttributeIDs.ATTR_CURRENT, unmngFeatureName))
                {
                    if (ReportMsg != null)
                    {
                        ReportMsg(this, new ReportMessage("Error while getting feature name", MsgTypes.MSG_ERROR));
                    }
                    //unallocate the memory and return
                    Marshal.FreeHGlobal(unmngFeatureName);
                    unmngFeatureName = IntPtr.Zero;
                ...
                featName = Marshal.PtrToStringAnsi(unmngFeatureName, PvTypes.PARAM_NAME_LEN);
                ...
                if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_FEAT_ID, (Int16)PvTypes.AttributeIDs.ATTR_CURRENT, unmngFeaturID))
                {
                    if (ReportMsg != null)
                    {
                        ReportMsg(this, new ReportMessage("Error while getting feature ID", MsgTypes.MSG_ERROR));
                    }
                    //unallocate the memory and return
                    Marshal.FreeHGlobal(unmngFeaturID);
                    unmngFeaturID = IntPtr.Zero;
                ...
                featID = (UInt32)Marshal.ReadInt32(unmngFeaturID);

                //free unmanaged pointers
                Marshal.FreeHGlobal(unmngFeaturID);
                unmngFeaturID = IntPtr.Zero;

                if (featID == (uint)targetfeatID)
                {
                    bfeatFound = true;
                    break;
                }
            ...
            if (bfeatFound == false)
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Target Post processing feature not found", MsgTypes.MSG_ERROR));
                }
                return;
            }
            ...
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Found target Feature at feature Index = {0}, Feature Name = {1}, Feature ID = {2}", featIndex, featName, featID), MsgTypes.MSG_ERROR));
            }

            //Determine number of Functions on this feature has
            IntPtr unmgFuctCount = Marshal.AllocHGlobal(sizeof(Int32));
            ...
            if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_PARAM_INDEX, (Int16)PvTypes.AttributeIDs.ATTR_COUNT, unmgFuctCount))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Error while getting function count", MsgTypes.MSG_ERROR));
                }
                //unallocate the memory and return
                Marshal.FreeHGlobal(unmgFuctCount);
                unmgFuctCount = IntPtr.Zero;
            ...
            funcCount = (UInt32)Marshal.ReadInt32(unmgFuctCount);
            ...
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Number of function in this feature : {0}", funcCount), MsgTypes.MSG_ERROR));
            }

            //Loop through all functions for this feature
            for (funcIndex = 0; funcIndex < funcCount; funcIndex++)
            {
                IntPtr unmngFuncIndex = Marshal.AllocHGlobal(sizeof(UInt32));
                Marshal.WriteInt32(unmngFuncIndex, funcIndex);
                if (!PVCAM.pl_set_param(m_hCam, PvTypes.PARAM_PP_PARAM_INDEX, unmngFuncIndex))
                {
                    if (ReportMsg != null)
                    {
                        ReportMsg(this, new ReportMessage("Error while setting function index", MsgTypes.MSG_ERROR));
                    }
                    //free unmanaged memory
                    Marshal.FreeHGlobal(unmngFuncIndex);
                    unmngFuncIndex = IntPtr.Zero;
                    return;
                }
                //free unmanaged memory
                Marshal.FreeHGlobal(unmngFuncIndex);
                ...
                //get feature name & index of that feature 
                IntPtr unmngFuncName = Marshal.AllocHGlobal(PvTypes.PARAM_NAME_LEN);

                if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_PARAM_NAME, (Int16)PvTypes.AttributeIDs.ATTR_CURRENT, unmngFuncName))
                {
                    if (ReportMsg != null)
                    {
                        ReportMsg(this, new ReportMessage("Error while getting function name", MsgTypes.MSG_ERROR));
                    }
                    //unallocate the memory and return
                    Marshal.FreeHGlobal(unmngFuncName);
                    unmngFuncName = IntPtr.Zero;
                    return;
                }

                funcName = Marshal.PtrToStringAnsi(unmngFuncName, PvTypes.PARAM_NAME_LEN);
                ...
                funcID = (UInt32)Marshal.ReadInt32(unmngFunctionID);
                //free unmanaged pointers
                Marshal.FreeHGlobal(unmngFunctionID);
                unmngFunctionID = IntPtr.Zero;

                if (funcID == (uint)targetfuncID)
                {
                    bfuncFound = true;
                    break;
                }
            ...
            if (bfuncFound == false)
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Target Post processing function not found", MsgTypes.MSG_ERROR));
                }
                return;
            }
            ...
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Found target function at Function Index = {0}, Functions Name = {1}, Function ID = {2}", funcIndex, funcName, funcID), MsgTypes.MSG_STATUS));
            }
            ...
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Setting function value to : {0}", newValue), MsgTypes.MSG_STATUS));
            }

            IntPtr unmngNewValue = Marshal.AllocHGlobal(sizeof(UInt32));
            Marshal.WriteInt32(unmngNewValue, newValue);
            if (!PVCAM.pl_set_param(m_hCam, PvTypes.PARAM_PP_PARAM, unmngNewValue))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Error while setting function value", MsgTypes.MSG_ERROR));
                }
                //free unmanaged memory
                Marshal.FreeHGlobal(unmngNewValue);
                unmngNewValue = IntPtr.Zero;
                return;
            }
            //free unmanaged memory
            Marshal.FreeHGlobal(unmngNewValue);
            ...
            //optional read back to make sure
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage("Reading back current value", MsgTypes.MSG_STATUS));
            }

            IntPtr unmngCurValue = Marshal.AllocHGlobal(sizeof(Int32));
            if (!PVCAM.pl_get_param(m_hCam, PvTypes.PARAM_PP_PARAM, (Int16)PvTypes.AttributeIDs.ATTR_CURRENT, unmngCurValue))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Error while reading back function value", MsgTypes.MSG_ERROR));
                }
                //unallocate the memory and return
                Marshal.FreeHGlobal(unmngCurValue);
                unmngCurValue = IntPtr.Zero;
                return;
            }
            curValue = (UInt32)Marshal.ReadInt32(unmngCurValue);
            //free unmanaged pointers
            Marshal.FreeHGlobal(unmngCurValue);
            unmngCurValue = IntPtr.Zero;
            ...
            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(string.Format("Current value of the fuction : {0}", curValue), MsgTypes.MSG_STATUS));
            }
        ...
        public void ReadCameraParams()
        {
            ...
            m_FirmwareMajorVersion = fwHighByte;
            m_FirmwareMinorVersion = fwLowByte;
        ...
        public void SetClearMode(string clearMode)
        {
            ...
            //if corresponding value not found, set default to Pre-Exposure (1).
            if (i >= m_clearModeList.Count)
            {
                int value = 1;  //Pre-Exposure = 1;
                Marshal.WriteInt32(unmngClearMode, value);
            }
        ...
        public void SetClockingMode(string clockingMode)
        {
            ...
            //if corresponding value not found, set default to "Normal"
            if (i >= m_clockModeList.Count)
            {
                int value = 0;  //Normal = 0;
                Marshal.WriteInt32(unmngClockMode, value);
            }
        ...
        public bool SetGainState(Int16 gainState)
        {
            ...
            Marshal.WriteInt16(unmngGainState, gainState);
        ...
        public unsafe void FrameToBitmap(ushort[] dataToConvert, Int32 width, Int32 height)
        {
            m_lastBMP = null;
            m_lastBMP = new WriteableBitmap(width, height, 96, 96, PixelFormats.Gray16, null);
            //int iStride = ((width * 16) + 31) / 32 * 4;   //Getting "Buffer size is not sufficient" with this stride calculation
            int iStride = (width * 16 + 7) / 8;
            m_lastBMP.WritePixels(new Int32Rect(0, 0, width, height), dataToConvert, iStride, 0);
        }
        ...
        public void GetFirmwareVersion()
        {
            ...
            //UInt16 fwHighByte = (UInt16)((firmwareVersion & 0xFF00) >> 8);
            //UInt16 fwLowByte = (UInt16)(firmwareVersion & 0xFF);
            m_FirmwareMajorVersion = (UInt16)((firmwareVersion & 0xFF00) >> 8);
            m_FirmwareMinorVersion = (UInt16)(firmwareVersion & 0xFF);
            Marshal.FreeHGlobal(unmngFwVersion);

            if (ReportMsg != null)
            {
                ReportMsg(this, new ReportMessage(String.Format("Camera firmware: {0}.{1}", m_FirmwareMajorVersion, m_FirmwareMinorVersion), MsgTypes.MSG_STATUS));
            }
        }

        public void SetShutterMode(PvTypes.ShutterModes shutterMode)
        {
            IntPtr unmngShutterOpenMode;
            unmngShutterOpenMode = Marshal.AllocHGlobal(sizeof(Int32));

            Marshal.WriteInt32(unmngShutterOpenMode, (Int32)shutterMode);

            if (!PVCAM.pl_set_param(m_hCam, PvTypes.PARAM_SHTR_OPEN_MODE, unmngShutterOpenMode))
            {
                if (ReportMsg != null)
                {
                    ReportMsg(this, new ReportMessage("Set shutter mode failed", MsgTypes.MSG_ERROR));
                }
            }

            Marshal.FreeHGlobal(unmngShutterOpenMode);
            unmngShutterOpenMode = IntPtr.Zero;
        }
        ...
        public WriteableBitmap LastBMP
        {
            get { return m_lastBMP; }
            set { m_lastBMP = value; }
        }
        ...
        public UInt32 ExposureTime
        {
            ...
            set
            {
                ...
                {
                    //Xceed.Wpf.Toolkit.MessageBox.Show("Please, enter value from 0 ms to 100000 ms", "Parameter incorrect");
                    m_exposureTime = 10;
                }
            }
        }
        ...
        public Boolean IsDefectivePixelCorrection
        {
            get { return m_isDefectivePixelCorrection; }
        }

        public Boolean IsDynamicDarkFrameCorrection
        {
            get { return m_isDynamicDarkFrameCorrection; }
        }

        public Boolean IsEnhancedDynamicRange
        {
            get { return m_isEnhancedDynamicRange; }
        }
        ...
        {
            get { return m_acqRunning; }
        }

        public UInt16 FirmwareMajorVersion
        {
            get { return m_FirmwareMajorVersion; }
        }

        public UInt16 FirmwareMinorVersion
        {
            get { return m_FirmwareMinorVersion; }
        }

        public string PvCamFirmwareVersion
        {
            get
            {
                string firmmwareVersion = string.Format("{0}.{1}", m_FirmwareMajorVersion, m_FirmwareMinorVersion);
                return firmmwareVersion;
            }
        }

        public PvTypes.ExposureModes TriggerMode
        {
            get { return (PvTypes.ExposureModes)m_triggerMode; }
            set
            {
                m_triggerMode = (int)value;
            }
        }
    }
